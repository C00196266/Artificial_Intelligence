...
*/
	...
	for (int i = 0; i < v.size(); i++) {
		if (location.x != v.at(i).location.x && location.y != v.at(i).location.y) {
			R = Pvector(location.x - v.at(i).location.x, location.y - v.at(i).location.y);

			D = R.magnitude();

			if (D < 250 && D > 0) {
				U = (-150/ pow(D, 2)) + (3000 / pow(D, 3));

				R.normalize();

				sum = Pvector(sum.x + (R.x * U), sum.y + (R.y * U));
			}
		}
	}
