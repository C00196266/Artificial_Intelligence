...
AlienArrive::AlienArrive(float maxSpeed) {
	if (!m_image.loadFromFile("Alien Arrive.png")) {
...
void AlienArrive::update(sf::Vector2f maxPos, sf::Vector2f target, sf::Time time) {
	...
	if (m_distance < m_arrivalRadius) {
		...
		m_targetSpeed = m_maxSpeed * (m_distance / m_slowRadius);
	}

	m_targetVel = m_direction;

	m_targetVel = normalise(m_targetVel);

	m_targetVel *= m_targetSpeed;
...
void AlienArrive::draw(sf::RenderWindow &window) {
	window.draw(m_sprite);
}

sf::Vector2f AlienArrive::normalise(sf::Vector2f v) {
