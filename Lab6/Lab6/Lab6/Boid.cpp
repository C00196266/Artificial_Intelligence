...
// =============================================== //
// ======== Boid Functions from Boid.h =========== //m_leader->update(deltaTime);
// =============================================== //
...
Pvector Boid::Separation(vector<Boid> boids)
{
	...
	for (int i = 0; i < boids.size(); i++)
	{
		...
		else if ((d > 0) && (d < desiredseparation) && predator == true && boids[i].predator == true)
		{
...
Pvector Boid::seek(Pvector v)
{
	...
	return acceleration; // this returns 0 - doesn't seem to be intended at all
}

void Boid::predatorSeek(Pvector target, vector<Boid> &b) {
	Pvector sub(0, 0);
	sub = sub.subTwoVector(target, location);
	int eatenIndex = -1;
	float d = sub.magnitude();;

	for (int i = 0; i < b.size(); i++) {
		if (b[i].predator == false) {
			Pvector prey = sub.subTwoVector(b[i].location, location);
			float preyD = prey.magnitude();
...
{
	if (underwater == true) {
		if (location.x < 0) location.x += 1920;
		if (location.y < 520) applyForce(Pvector(0, 20000));
		if (location.x > 1920) location.x -= 1920;
		if (location.y > 1050) applyForce(Pvector(0, -100));
	}
	else {
		if (location.x < 0) location.x += 1920;
		if (location.y < 10) applyForce(Pvector(0, 100));
		if (location.x > 1920) location.x -= 1920;
		if (location.y > 495) applyForce(Pvector(0, -100));
	}
}
